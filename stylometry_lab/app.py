...
if analysis_mode.startswith("Single"):
    ...
    try:
        if feat and isinstance(feat, dict):
            ...
            feat_sorted = dict(sorted(feat.items()))

            # Create dataframe
            df_feat = pd.DataFrame([
                {"Feature": k.replace("_", " ").title(), "Value": f"{v:.4f}" if isinstance(v, float) else str(v)}
                for k, v in feat_sorted.items()
            ])

            st.dataframe(df_feat, use_container_width=True)
            st.success(f"✅ Extracted {len(feat)} stylistic features")
        else:
            st.warning(f"⚠️ Features not available. Type: {type(feat)}, Value: {feat}")
    ...
    try:
        if stab and isinstance(stab, dict) and len(stab) > 0:
            df_stab = pd.DataFrame([
                {"Feature": k.replace("_", " ").title(), "Stability": f"{v:.4f}"}
                for k, v in stab.items()
            ]).sort_values("Stability", ascending=False)
            st.dataframe(df_stab, use_container_width=True)
        else:
            st.info("ℹ️ Stability metrics not available")
    except Exception as e:
        st.error(f"❌ Error displaying stability: {str(e)}")
